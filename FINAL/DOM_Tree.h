...
class DOM_Tree
{
    private:
        ...
        void arbolConHijos(Element e, list<DOM_Tree> hijos, DOM_Tree &h);
...
DOM_Tree::DOM_Tree(Element e, list<DOM_Tree> hijos)
{
    ...
    while(!hijos.empty())
    {
        arbol.appendChild(hijos.front());
        hijos.pop_front();
    }
    this->raiz = copiar(arbol.raiz);

}

 void DOM_Tree::arbolConHijos(Element e, list<DOM_Tree> hijos, DOM_Tree &h)
 {
